...
[cb1f:p]
  ...
  [46ed:s] I hit that failure while drafting an essay I now call source engineering, so the writing system itself became evidence for the larger thesis.
...
[6219:p]
  [6017:s] This workflow does not divide cleanly into drafting and editing.
...
[8175:p]
  [169f:s] I call the broader line source engineering: it treats parsing, normalization, linkage, verification, and release as phases of one compilation problem concerned with historical intelligibility rather than artifacts alone.
[518b:p]
  [984f:s] Pandoc [@MacFarlane_Pandoc] was a direct source of inspiration here: it made writing legible as a compilation and build pipeline with intermediate structure, transforms, and multiple output surfaces.
  [9957:s] That also made Haskell feel like a plausible implementation language for knots, given Pandoc's own ecosystem and the typed, compiler-shaped character of the problem.
[0372:p]
  [b723:s] Steve Yegge's [beads](https://github.com/steveyegge/beads) supplied a more concrete architectural influence: it pulled planning and coordination back into the repository itself through a local-first model rather than a hosted workflow.
...
[ea82:p]
  [9505:s] The eventual name came through C. L. R. James's Hegelian use of knots: formations that harden out of multiplicity into points of arrest and direction [@jamesNotesOnDialectics1948].
  [cda7:s] Knots was closer to the problem than files, nodes, or blocks because the system needed to preserve those local stabilizations of meaning, reference, and revision.
[e819:h2] Failure
[6b17:p]
...
[7713:p]
  ...
  [c121:s] Darcs and later Pijul are closer precedents here than Git, because they treat patches or changes as primary historical objects, reason explicitly about commutation and dependence, and represent conflict as part of the history rather than only as a failed merge against snapshots [@darcsMergersDocumentation; @darcsConflictors; @pijulWhyPijul].
[eaa5:h2] Claims
[909e:p]
...
[93b1:p]
  ...
  [f604:s] Review and promotion semantics, fuller branching, build-system integration, and networked source infrastructure belong to the larger line, but not to this memo.
[0ea4:p]
...
[6746:p]
  ...
  [9fc6:s] Email: [email](mailto:codexjournalinquiries@gmail.com). Instagram: [codexjournal](https://instagram.com/codexjournal). X: [sourceaware](https://x.com/sourceaware). GitHub: [codex-journal](https://github.com/codex-journal).
